  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className={styles.bar}>
        ...
        <Button
          label={"Search"}
          callback={() => console.log(`Term Searched ${value}`)}
        />
      </form>
      <br />

      {loading ? (
        <LoadingMessage message={"Loading Mercado Libre Section"} />
      ) : null}
        ...
        {products.map((prod) => {
          return (
            <div className={stylesc.card} key={prod.id}>
              ...
              <p className={stylesc.cardText}>
                <b>Qty:</b>1
              </p>
              <Button
                label={"Add To Cart"}
                color={"blue"}
                callback={() => console.log(`Adding product ${prod.title}`)}
              />
            </div>
          );
        })}
